...
class STM:
    ...
    def search(self, query, search_mode=True, return_buffer=True):
        try: 
            if self.mode.lower() == 'normal':
                ...
                if not search_mode:
                    buffer_results = []
                    for item in reversed(self.buffers): 
                    ...
                    return buffer_results, [], ('', np.zeros(self.Generator.Encoder_dim, dtype=np.float32).reshape(1, -1))
                ...
                for i, ids in enumerate(indices[0] if indices.size > 0 else []):
                    if ids != -1 and ids in self.Memory:
                        item = self.Memory[ids].copy()
                        item['similarity'] = float(distances[0][i]) if distances.size > 0 else 0.0
                        item['id'] = int(ids)
                        results.append(item)
                ...
                if return_buffer and self.buffers:
                    for item in list(self.buffers)[-5:]:
                        buffer_embedding = self.Generator.Get_Embedding(item['value']) 
                        sim = self._cosine_similarity(Query_embedding, buffer_embedding)
    ...
    def add(self, query: str, tool_result: str = ''):
        try:
            ...
            logger.info(f"[BUFFER] Added to buffer. Size: {len(self.buffers)}")
            
            if self.mode.lower() == 'normal':
                cleaned_query = clean_text(query)
                query_embedding = self.Generator.Get_Embedding(f"passage: {cleaned_query}")
                
                if self.index.ntotal > 0:
                    distances, _ = self.index.search(query_embedding, k=1)
                    if len(distances) > 0 and distances[0][0] >= 0.99:
                        logger.info(f"[STM] Duplicate detected (sim={distances[0][0]:.3f}), not adding")
                        return False
    ...
    def load(self):
        ...
        try:
            with open(self.Path, 'rb') as f:
                ...
                self.buffers = deque(data.get('buffer', []), maxlen=10)
            self.index = faiss.read_index(f"{self.Path}_index.faiss")
            logger.info(f"[LOADER] Loaded {len(self.Memory)} items and {len(self.buffers)} buffer items")
        except Exception as e:
